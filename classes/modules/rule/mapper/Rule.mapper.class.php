<?php
/* ---------------------------------------------------------------------------
 * @Project: Alto CMS
 * @Plugin Name: Magic Rules
 * @Description: Specific rules for voting and add content
 * @Author: Alto CMS Team
 * @Author URI: http://altocms.com
 * @License: GNU GPL v2
 *----------------------------------------------------------------------------
 * Based on
 *   Plugin Magic Rule for LiveStreet CMS
 *   Author: LiveStreet Developers Team
 *   Site: https://github.com/livestreet/lsplugin-magicrule
 *----------------------------------------------------------------------------
 */

class PluginMagicrules_ModuleRule_MapperRule extends MapperORM {

    /**
     * @param int    $iUserId
     * @param string $sTargetType
     * @param string $sDate
     *
     * @return int
     */
    public function GetCountVote($iUserId, $sTargetType, $sDate) {

        $sql
            = "
			SELECT count(*) as c FROM ?_vote
			WHERE
				user_voter_id = ?
				AND
				target_type = ?
				AND
				vote_date >= ?
		";
        if ($aRow = $this->oDb->selectRow($sql, $iUserId, $sTargetType, $sDate)) {
            return $aRow['c'];
        }
        return 0;
    }

    /**
     * @param int    $iUserId
     * @param string $sDate
     *
     * @return int
     */
    public function GetSumRatingTopic($iUserId, $sDate = null) {

        $sql
            = "
			SELECT sum(topic_rating) as s FROM ?_topic
			WHERE
				user_id = ?
				AND
				topic_publish = 1
				{ AND topic_date_add >= ? }
		";
        if ($aRow = $this->oDb->selectRow($sql, $iUserId, $sDate ? $sDate : DBSIMPLE_SKIP)) {
            return $aRow['s'] ? $aRow['s'] : 0;
        }
        return 0;
    }

    /**
     * @param int    $iUserId
     * @param string $sDate
     *
     * @return int
     */
    public function GetSumRatingComment($iUserId, $sDate = null) {

        $sql
            = "
			SELECT sum(comment_rating) as s FROM ?_comment
			WHERE
				user_id = ?
				AND
				target_type = 'topic'
				AND
				comment_publish = 1
				AND
				comment_delete = 0
				{ AND comment_date >= ? }
		";
        if ($aRow = $this->oDb->selectRow($sql, $iUserId, $sDate ? $sDate : DBSIMPLE_SKIP)) {
            return $aRow['s'] ? $aRow['s'] : 0;
        }
        return 0;
    }

}

// EOF